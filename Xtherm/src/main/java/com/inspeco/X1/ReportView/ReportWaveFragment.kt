...
import kotlinx.android.synthetic.main.fragment_report_wave.view.*


class ReportWaveFragment() : Fragment() {
    private val TAG = "bobopro-ReportWaveFragment"
    ...
    override fun onCreateView(
        ...
        val view: View = inflater!!.inflate(R.layout.fragment_report_wave, container, false)
        mView = view

        view.openUdrButton.setOnClickListener{
            val dialog = FileListDialog(mContext, Consts.UDR_WAVE_FOLDER)
            if (dialog.fileList.size > 0) {
                ...
                dialog.setFileClickListener {
                    ...
                    var intent = Intent(mContext, ReportWaveResultActivity::class.java)
        ...
        view.openReportButton.setOnClickListener {
            val dialog = FileListDialog(mContext, Consts.REPORT_WAVE_FOLDER)
            if (dialog.fileList.size > 0) {
                val windowManager = mContext.getSystemService(Context.WINDOW_SERVICE) as WindowManager
                val display = windowManager.defaultDisplay
                val size = Point()
