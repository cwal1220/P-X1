...
import kotlinx.android.synthetic.main.fragment_report_mix.view.*


class ReportMixFragment() : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view: View = inflater!!.inflate(R.layout.fragment_report_mix, container, false)
        mView = view


        view.openUdrButton.setOnClickListener{
            val dialog = FileListDialog(mContext, Consts.UDR_MIX_FOLDER)
            if (dialog.fileList.size > 0) {
                ...
                dialog.setFileClickListener {
                    ...
                    var intent = Intent(mContext, ReportMixResultActivity::class.java)
        ...
        view.openReportButton.setOnClickListener {
            val dialog = FileListDialog(mContext, Consts.REPORT_MIX_FOLDER)
            if (dialog.fileList.size > 0) {
                val windowManager = mContext.getSystemService(Context.WINDOW_SERVICE) as WindowManager
                val display = windowManager.defaultDisplay
                val size = Point()
