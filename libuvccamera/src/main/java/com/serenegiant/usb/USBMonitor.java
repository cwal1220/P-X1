...
public final class USBMonitor {
	...
	private static final String TAG = "bobopro USBMonitor";
	...
	public synchronized boolean requestPermission(final UsbDevice device) {
		...
		if (isRegistered()) {
			if (device != null) {
				...
				if (DEBUG) Log.v(TAG, "check device=" + device);
				if (mUsbManager.hasPermission(device)) {
					// call onConnect if app already has permission
					if (DEBUG) Log.v(TAG, "OK device=" + device);
					processConnect(device);
				} else {
					try {
						// パーミッションがなければ要求する
						if (DEBUG) Log.v(TAG, "need Req Permission" + device);
						mUsbManager.requestPermission(device, mPermissionIntent);
					} catch (final Exception e) {
						// Android5.1.xのGALAXY系でandroid.permission.sec.MDM_APP_MGMTという意味不明の例外生成するみたい
						if (DEBUG) Log.v(TAG, "Req Permission Exceped");
						Log.w(TAG, e);
